...
<template>
  <div class="json-lab">
    ...
    <section class="lab-section">
      ...
      <ul>
        <li v-for="(store, index) in storeTypes" :key="index">
      ...
      <ul>
        <li v-for="(entry, index) in openingHours" :key="index">
          {{ entry.dayType }}: {{ entry.open }} – {{ entry.close }}
        </li>
      </ul>
...
import bookstores from '../assets/json/bookstore.json'
...
const openingHours = computed(() => {
        return Object.entries(bookstores.openingHours).map(([dayType, hours]) => ({
          dayType,
          open: hours.open,
          close: hours.close
        }))
...
const countries = computed(() => bookstores.countries)
const showMessage = ref(false);
const storeTypes = computed(() =>
  Object.entries(bookstores.storeTypes).map(([type, count]) => ({
    type,
    count
  }))
)
